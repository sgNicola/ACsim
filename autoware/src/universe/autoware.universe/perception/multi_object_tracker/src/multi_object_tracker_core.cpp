...
: rclcpp::Node("multi_object_tracker", node_options),
  ...
  tracker_faulty_mode(declare_parameter<int>("tracker_faulty_mode"))
{
  ...
  tracker_timeLatencyDuration = declare_parameter<int>("tracker_timeLatencyDuration");
...
void MultiObjectTracker::publish(const rclcpp::Time & time) const
{
  ...
  int fault_mode = tracker_faulty_mode;
  int timeLatency = tracker_timeLatencyDuration;
  switch (fault_mode){
    case 0:
      for (auto itr = list_tracker_.begin(); itr != list_tracker_.end(); ++itr) {
    ...
    case 2:
      for (auto itr = list_tracker_.begin(); itr != list_tracker_.end(); ++itr) {
        ...
        autoware_auto_perception_msgs::msg::TrackedObject object;
        (*itr)->getTrackedObject(time, object);
        object.kinematics.pose_with_covariance.pose.position.x +=5;
        object.kinematics.pose_with_covariance.pose.position.y +=1;
        output_msg.objects.push_back(object);
      }
      break;
    case 3:
