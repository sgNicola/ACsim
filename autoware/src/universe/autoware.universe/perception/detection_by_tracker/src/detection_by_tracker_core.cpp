  ...
  tf_listener_(tf_buffer_)
{
  ...
  dbt_faulty_mode = declare_parameter<int>("dbt_faulty_mode");
  dbt_timeLatencyDuration = declare_parameter<int>("dbt_timeLatencyDuration");
  ...
  const tier4_perception_msgs::msg::DetectedObjectsWithFeature::ConstSharedPtr input_msg)
{
  ...
  int faulty_mode = dbt_faulty_mode;
  int timeLatency = dbt_timeLatencyDuration;
  ...
  switch (faulty_mode){
    ...
    case 1:
      ...
      break;
    case 2:           
      if (!detected_objects.objects.empty()) {
      autoware_auto_perception_msgs::msg::DetectedObject ghost_object = detected_objects.objects.back();
      ...
      detected_objects.objects.push_back(ghost_object);
      }
      objects_pub_->publish(detected_objects);
      break;
    case 3:
      if (!detected_objects.objects.empty()) {
        auto& last_object = detected_objects.objects.back();;
