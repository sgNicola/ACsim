...
: Node("tensorrt_yolo", options)
{
  ...
  yolo_faulty_mode = declare_parameter<int>("yolo_faulty_mode");
  yolo_timeLatencyDuration = declare_parameter<int>("yolo_timeLatencyDuration");
...
void TensorrtYoloNodelet::callback(const sensor_msgs::msg::Image::ConstSharedPtr in_image_msg)
{
  ...
  int faulty_mode = yolo_faulty_mode;
  int timeLatency = yolo_timeLatencyDuration;
  ...
  switch (faulty_mode){
    case 0:
      ...
      break;
    case 1:
      out_objects.feature_objects.clear();
      objects_pub_->publish(out_objects);      
      break;
    case 2:           
      if (!out_objects.feature_objects.empty()) {
      tier4_perception_msgs::msg::DetectedObjectWithFeature ghost_object = out_objects.feature_objects.back();
      ...
      out_objects.feature_objects.push_back(ghost_object);
      }
      objects_pub_->publish(out_objects); 
      break;
    case 3:
      if (!out_objects.feature_objects.empty()) {
        auto& last_object = out_objects.feature_objects.back();
