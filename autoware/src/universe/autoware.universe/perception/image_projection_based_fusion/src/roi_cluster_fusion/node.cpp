...
: FusionNode<DetectedObjectsWithFeature, DetectedObjectWithFeature>("roi_cluster_fusion", options)
{
  ...
  fusion_timeLatencyDuration = declare_parameter<int>("fusion_timeLatencyDuration");
}
  ...
  const sensor_msgs::msg::CameraInfo & camera_info, DetectedObjectsWithFeature & output_cluster_msg)
{
  ...
  int timeLatency = fusion_timeLatencyDuration;
    ...
    switch (faulty_mode){
      ...
      case 3:
        if (!output_cluster_msg.feature_objects.empty()) {
          ...
          last_object.object.kinematics.pose_with_covariance.pose.position.x += 5;
          last_object.object.kinematics.pose_with_covariance.pose.position.y += 1;
        }
        debug_image_rois.push_back(feature_obj.feature.roi);
        break;
      case 4:
