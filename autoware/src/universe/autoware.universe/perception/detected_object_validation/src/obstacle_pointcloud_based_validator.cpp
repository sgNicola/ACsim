  ...
  sync_(SyncPolicy(10), objects_sub_, obstacle_pointcloud_sub_)
{
  ...
  validation_faulty_mode = declare_parameter<int>("validation_faulty_mode");
  ...
  const sensor_msgs::msg::PointCloud2::ConstSharedPtr & input_obstacle_pointcloud)
{
  ...
  int faulty_mode = validation_faulty_mode;
  switch (faulty_mode){
    case 0:
    ...
    case 2:
      ...
      autoware_auto_perception_msgs::msg::DetectedObject ghost_object = output.objects.back();
      ...
      output.objects.push_back(ghost_object);
    }
      objects_pub_->publish(output);
      break;
    case 3:
      if (!output.objects.empty()) {
        auto& last_object = output.objects.back();
        last_object.kinematics.pose_with_covariance.pose.position.x += 5;
        last_object.kinematics.pose_with_covariance.pose.position.y += 1;
       }
      objects_pub_->publish(output);    
      break;
  }
