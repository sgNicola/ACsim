...
import shutil
import sys
from perceptionIdentify.cluster_launch import modify_and_save_xml
from perceptionIdentify.process_rosbag import process_rosbag_file
from param import Rosbag_dir,setup_path,carla_path,lidar_launch_path,cluster_launch, mutation_runner,agent,cluster_intervene,ObjectData_dir
...
def run_carla_scenario_agent(experiment_id, bag_name,scenario,params,targets=None):
    ...
    input_file_path =  cluster_launch
    ...
    if targets is not None:
        ...
        intervene_command = f'gnome-terminal -- bash -c "source {setup_shell}; python3 {cluster_intervene} --targets {targets_str}"'
    ...
    time.sleep(4)
    bag_record_cmd = generate_bag_record_command(topics["cluster_topics"],bag_name)
