...
class ScenarioRunner(object):
    ...
    def _load_and_run_scenario(self, config):
        ...
        self._cleanup()
        return result
    def mutate_scenario_configuration(self, config):
        """
        Mutate scenario configuration parameters randomly within a predefined range.
        """
        # Define the range for mutation
        x_range = range(-10, 10)
        y_range = [-1.4, -1.2, -1, 0, -0.8, -0.6, -0.4, -0.2, 0, 0.2, 0.4, 0.6, 0.8, 1, 1.2, 1.4]
        yaw_range = (0, 360)
        # Randomly mutate the position and orientation
        config.X1 = random.uniform(*x_range)
        config.Y1 = random.uniform(*y_range)
        config.YAW1 = random.uniform(*yaw_range)
        config.X2 = random.uniform(*x_range)
        config.Y2 = random.uniform(*y_range)
        config.YAW2 = random.uniform(*yaw_range)
        config.X3 = random.uniform(*x_range)
        config.Y3 = random.uniform(*y_range)
        config.YAW3 = random.uniform(*yaw_range)
        config.weather_type = random.choice(config.weather_types)
        config.car_type = random.choice(config.car_types)
        config.truck_type = random.choice(config.truck_types)
        config.road_obstacle_type = random.choice(config.road_obstacle_types)
        config.sparse_obstacle_type = random.choice(config.sparse_obstacle_types)
        config.hugetruck_type = random.choice(config.hugetruck_types)
        config.buses_type = random.choice(config.buses_types)
        config.motorcycle_type = random.choice(config.motorcycle_types)
        config.vans_type = random.choice(config.vans_types)
        config.cyclist_type = random.choice(config.cyclist_types)
        config.pedestrian_type = random.choice(config.pedestrian_types)
        config.vehicle_color = random.choice(config.vehicle_colors)   
         
    def _run_scenarios(self,mutate=False):
        """
        ...
        for config in scenario_configurations:
            for _ in range(self._args.repetitions):
                self.finished = False
                # if mutate:
                #     config = self.mutate_scenario_configuration(config)
                config.X1 = float(self._args.X1)
                config.Y1 = float(self._args.Y1)
                config.X2 = float(self._args.X2)
                config.Y2 = float(self._args.Y2)
                config.YAW1 = float(self._args.YAW1)
                # config.car_type = self._args.object_type
                config.truck_type = self._args.object_type
                config.road_obstacle_type = self._args.object_type
                config.sparse_obstacle_type =  self._args.object_type
                config.hugetruck_type =  self._args.object_type
                config.buses_type =  self._args.object_type
                config.motorcycle_type = self._args.object_type
                config.vans_type =  self._args.object_type
                config.cyclist_type =  self._args.object_type
                config.pedestrian_type = self._args.object_type
...
def main():
    ...
    parser.add_argument('--X1', default=0, help='Provide an additional scenario configuration')
    parser.add_argument('--Y1', default=0, help='Provide an additional scenario configuration')
    parser.add_argument('--X2', default=0, help='Provide an additional scenario configuration')
    parser.add_argument('--Y2', default=0, help='Provide an additional scenario configuration')
    parser.add_argument('--YAW1', default=0, help='Provide an additional scenario configuration')
    parser.add_argument('--object_type', default='static.prop.constructioncone', help='Provide an additional scenario configuration')
    parser.add_argument('--color', default='(255, 255, 255)', help='Provide an additional scenario configuration')
