...
class CollisionTest(Criterion):
    ...
    MIN_AREA_OF_COLLISION = 1       # If closer than this distance, the collision is ignored
...
class HardBreakTest(Criterion):
    ...
    def __init__(self, actor, name="HardBreakTest", terminate_on_failure=False):
        """
        """
        super(HardBreakTest, self).__init__(name, actor, 0, terminate_on_failure=terminate_on_failure)
        self.logger.debug("%s.__init__()" % (self.__class__.__name__))
        self._actor = actor
        self._world = CarlaDataProvider.get_world()
        self._map = CarlaDataProvider.get_map()
