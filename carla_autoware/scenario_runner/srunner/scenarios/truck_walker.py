...
class Truck_walker(BasicScenario):
    ...
    timeout = 20000          # Timeout of scenario in seconds
    ...
    def __init__(self, world, ego_vehicles, config, randomize=False, debug_mode=False, criteria_enable=True):
        ...
        self._first_actor_location = 18
        # self._second_actor_location = 
        ...
        super(Truck_walker, self).__init__("Truck_walker",
                                                               ego_vehicles,
                                                               config,
                                                               world,
                                                               debug_mode,
    ...
    def _initialize_actors(self, config):
        ...
        try:
            first_actor = CarlaDataProvider.request_new_actor(
                config.car_type, self._first_actor_transform
            )
        ...
        self._second_actor_transform = carla.Transform(
            carla.Location(self._first_actor_transform.location.x-3,
                        self._first_actor_transform.location.y-1,
                        self._first_actor_transform.location.z),
            carla.Rotation(first_actor_waypoint.transform.rotation.pitch, first_actor_waypoint.transform.rotation.yaw + self.config.YAW1,
                        first_actor_waypoint.transform.rotation.roll))
 
        # spawn the first actor
        print(config.pedestrian_type)
        try:
            second_actor = CarlaDataProvider.request_new_actor(
                config.pedestrian_type, self._second_actor_transform
            )
        except Exception as e:
            # Handle the exception here, such as logging an error message or taking alternative actions
            print(f"Failed to create first actor: {e}")
            self._second_actor_transform = carla.Transform(
            carla.Location(first_actor_waypoint.transform.location.x-4,
                        first_actor_waypoint.transform.location.y + 0,
                        first_actor_waypoint.transform.location.z),
            carla.Rotation(first_actor_waypoint.rotation.pitch, first_actor_waypoint.transform.rotation.yaw + self.config.YAW1,
                        first_actor_waypoint.transform.rotation.roll))
            second_actor = CarlaDataProvider.request_new_actor(
                config.pedestrian_type, self._second_actor_transform
            )
