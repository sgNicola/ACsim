...
class TrafficCone(BasicScenario):
    ...
    def __init__(self, world, ego_vehicles, config, randomize=False, debug_mode=False, criteria_enable=True):
        ...
        self._second_actor_location = 10 
        self._third_actor_location = 20
        ...
        super(TrafficCone, self).__init__("TrafficCone",
                                                               ego_vehicles,
                                                               config,
                                                               world,
                                                               debug_mode,
    ...
    def _initialize_actors(self, config):
        ...
        self._third_actor_transform = carla.Transform(
            carla.Location(third_actor_waypoint.transform.location.x + config.X3,
                           third_actor_waypoint.transform.location.y + config.Y3,
        ...
        try:
            first_actor = CarlaDataProvider.request_new_actor(
                random.choice(config.car_type), self._first_actor_transform
            )
        ...
        # spawn the third actor
        try:
            third_actor = CarlaDataProvider.request_new_actor(
                config.road_obstacle_type, self._third_actor_transform
            )
        except Exception as e:
    ...
    def _create_behavior(self):
        ...
        sequence.add_child(ActorTransformSetter(self.other_actors[1], self._third_actor_transform))
