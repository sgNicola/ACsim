...
class FollowLeadingVehicle(BasicScenario):
    ...
    def _initialize_environment(self, world):
        self.config.weather.cloudiness = float(50)
        self.config.weather.precipitation = float()
        self.config.weather.precipitation_deposits = float()
        self.config.weather.wind_intensity = float()
        self.config.weather.sun_azimuth_angle = float()
        self.config.weather.sun_altitude_angle = float()
        self.config.weather.fog_density = float(50)
        self.config.weather.fog_distance = float()
        self.config.weather.wetness = float()
...
class FollowLeadingVehicleWithObstacle(BasicScenario):
    ...
    timeout = 10000           # Timeout of scenario in seconds
